...
struct emit_assign_node : proto::transform<emit_assign_node<Evaluator>>
{
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        ...
        result_type operator()(typename impl::expr_param expr, typename impl::state_param state,
                               typename impl::data_param data) const
        {
            return binary_operator_expr(binary_op_tag::assign,
                                        Evaluator()(proto::left(expr), state, data),
                                        Evaluator()(proto::right(expr), state, data));
        }
    };
...
struct emit_tensor_node : proto::transform<emit_tensor_node<Evaluator>>
{
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
                               ...
                               typename impl::data_param data) const
        {
            ...
            std::shared_ptr<tensor_variable> tensor_var;

            auto handle = proto::value(proto::child_c<0>(expr)).data_handle();

            auto& symbol_table = state.get();

            auto iter = symbol_table.find(handle);

            if (iter != symbol_table.end())
            {
                tensor_var = iter->second;
            }
            else
            {
                tensor_var =
                    std::make_shared<tensor_variable>(handle, types::tensor(types::double_{}));
                symbol_table.emplace(handle, tensor_var);
            }

            auto tensor_expr = tensor_access_expr(tensor_var);
...
struct emit_AST
    : proto::or_<emit_logical_operations, emit_arithmetic_operations,
                 ...
                 proto::when<proto::assign<proto::_, proto::_>, emit_assign_node<emit_AST>>,
...
expression emit_ast(const Expr& expr)
{
    std::map<qbb::util::handle, std::shared_ptr<tensor_variable>> symbol_table;

    return emit_AST
    {
    }
    (expr, std::ref(symbol_table));
}
}
}
