...
hpx::future<void> round_robin_scheduler::schedule(const symbol_id& func, execution_context ctx)
{
    std::lock_guard<hpx::lcos::local::mutex> guard(scheduling_mutex_);

    auto resource_count = execution_resources_.size();

    auto execution_finished = execution_resources_[next_endpoint_]->execute(func, std::move(ctx));
