...
std::size_t abi_info::get_align_of(const type& QUBUS_UNUSED_RELEASE(primitive_type)) const
{
    QUBUS_ASSERT(primitive_type.is_primitive(), "'primitive_type' has to be primitive.");
...
std::size_t abi_info::get_size_of(const type& primitive_type) const
{
    ...
    auto m = pattern::make_matcher<type, std::size_t>()
                 .case_(pattern::double_t, [&] { return sizeof(double); })
                 .case_(pattern::float_t, [&] { return sizeof(float); })
                 .case_(pattern::integer_t, [&] { return sizeof(util::index_t); })
                 .case_(pattern::bool_t, [&] { return sizeof(bool); })
                 .case_(complex_t(real_type), [&] { return 2 * get_size_of(real_type.get()); });
                                        ...
                                        const std::vector<util::index_t>& shape) const
{
    ...
    return array_layout(current_size, block_alignment, shape_offset, shape_size, data_offset,
                        data_size);
}
}