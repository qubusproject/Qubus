...
: std::runtime_error("The symbol " + symbol_ + " has already be defined in module " + module_id_.string() +
                     "."),
...
: std::runtime_error("Failed to import module " + imported_module_id_ + " into " + module_id_.string() +
                     "."),
  module_id_(std::move(module_id_)),
...
void module::add_function(std::string name, std::vector<variable_declaration> params,
...
void load_construct_data(hpx::serialization::input_archive& ar, module* mod, unsigned QUBUS_UNUSED(version))
{
    ::new (mod) module();
}
