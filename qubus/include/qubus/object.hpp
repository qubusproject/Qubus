...
{
public:
    object_id(std::uint64_t id_msb_, std::uint64_t id_lsb_)
    : id_msb_(id_msb_), id_lsb_(id_lsb_)
    {
    }
    ...
    explicit object_id(const hpx::naming::gid_type& gid)
    : object_id(gid.get_msb(), gid.get_lsb())
    {
    }
