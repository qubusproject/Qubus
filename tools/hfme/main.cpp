                          ...
                          const qbb::tensor<std::complex<double>, 4>& w, const std::string& path)
{
    ...
    for (qbb::index_t i = 0; i < Nb; ++i)
    {
        for (qbb::index_t j = 0; j < Nb; ++j)
        {
            ...
            if (std::abs(imag(h(i, j))) > 1e-10)
            {
                std::cout << "warning: found non-vanishing imaginary part in one-electron integrals"
                          << std::endl;
            }
        }
        fout << "\n";
        ...
        for (qbb::index_t l = 0; l < Nb; ++l)
        {
            for (qbb::index_t j = 0; j < Nb; ++j)
            {
                for (qbb::index_t k = 0; k < Nb; ++k)
                {
                    ...
                    if (std::abs(imag(w(i, j, k, l))) > 1e-10)
                    {
                        std::cout << "warning: found non-vanishing imaginary part in two-electron "
                                     "integrals" << std::endl;
                    }
                }
            }
...
void dump_spatial_representation(const std::vector<double>& x,
                                 const qbb::tensor<std::complex<double>, 2>& transformation_matrix,
                                 const std::string& path)
{
    std::ofstream fout(path);

    qbb::index_t Nb = transformation_matrix.shape()[0];
    qbb::index_t Nx = transformation_matrix.shape()[1];

    for (qbb::index_t i = 0; i < Nb; ++i)
    {
        for (qbb::index_t j = 0; j < Nx; ++j)
        {
            fout << x[j] << "  " << real(transformation_matrix(i, j)) << "\n";
            
            if (std::abs(imag(transformation_matrix(i, j))) > 1e-10)
            {
                std::cout << "warning: found non-vanishing imaginary part in transformation matrix"
                          << std::endl;
            }
        }
    }
...
int main(int argc, char** argv)
{
    ...
    generic.add_options()("version", "print version string")("help", "produce help message")(
        ...
        "spatial-repr-file", bpo::value<std::string>(&spatial_repr_file),
        "path to the spatial representation file")("N_alpha", bpo::value<qbb::index_t>(&N_alpha),
                                                   "number of spin-up particles")(
        "N_beta", bpo::value<qbb::index_t>(&N_beta)->default_value(0),
        "number of spin-down particles")("lambda", bpo::value<double>(&lambda)->default_value(1.0),
    ...
    {
        std::cout
            << "error: only spin-polarized HF is implemented by now (N_alpha == 0 || N_beta == 0)"
            << std::endl;
    ...
    std::cout << "\tnumber of one-particle basis functions: "
              << loader.get_one_particle_hamiltonian().shape()[0] << std::endl;
    ...
    auto hf_one_particle_hamiltonian =
        hf_basis.transform_operator(loader.get_one_particle_hamiltonian());
    ...
    auto hf_two_particle_interaction_operator =
        hf_basis.transform_operator(loader.get_two_particle_interaction_operator());
    ...
    dump_matrix_elements(hf_one_particle_hamiltonian, hf_two_particle_interaction_operator,
                         "test_basis.dat");
    ...
    if (!spatial_repr_file.empty())
    {
        ...
        if (Nb != Nb_funcs)
        {
            std::cout << "error: number of basis function does not match the number of given "
                         "spatial representations (" << Nb << " != " << Nb_funcs << ")"
                      << std::endl;
            return -1;
        }
        ...
        auto hf_spatial_transform =
            hf_basis.concatenate_backtransforms(sp_loader.get_transformation_matrix());

        std::size_t Nx = hf_spatial_transform.shape()[1];

        auto hf_sp_trans_dataset =
            fout.create_dataset<std::complex<double>>("hf_spatial_transform", {Nb, Nx});
        hf_sp_trans_dataset <<= hf_spatial_transform;

        hf_sp_trans_dataset.dimensions()[1].relabel("x");
        auto sp_coords_scale =
            hf_sp_trans_dataset.dimensions()[1].attach_dimension_scale<double>("x");

        sp_coords_scale <<= sp_loader.get_spatial_coordinates();
        
        dump_spatial_representation(sp_loader.get_spatial_coordinates(), hf_spatial_transform, "test_transform.dat");
    }
    else
    {
